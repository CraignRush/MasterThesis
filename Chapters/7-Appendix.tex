...
For non-linear graph fitting, two distinct functions have been used: The Hill-function (\cref{eq:hill}) and the exponential decay (\cref{eq:exp}).
\begin{align}
	y &= V_\text{max}\frac{x^n}{k^n +x^n}	\label{eq:hill} \\
	y &= Ae^\frac{-x}{\tau_{\scriptscriptstyle\text{decay}}} + y_0 \label{eq:exp}
\end{align}
\clearpage
...
\begin{figure}[!h]
	...
	\capption{Measured Bead Velocity}{\Acrfull{vc} from the affinity-based concentration measurements in \cref{sec:res:affinity}. The p-Value is < \num{10e-2}}
	\label{fig:conc:vel}
\end{figure}
...
\begin{figure}
	\centering
	\subfloat{
		\subfigimg[angle=-90,origin=c,width=\linewidth]{a}{./Ressources/ResultPlots/Serpentine_1000um_2-FullLayout}
		\phantomsubcaption
		\label{fig:app:serpentine}
	} \\
	\vspace{\baselineskip}
	\addtocounter{subfigure}{-1}
	\subfloat{
		\subfigimg[width=\linewidth]{b}{./Ressources/ResultPlots/SerpentineApertureCapture30um2}  
		\phantomsubcaption
		\label{fig:app:focus}
	}
	\capption{AutoCAD Drawings of Microfluidic Channels}{Scale: \SI{20}{\milli\meter}:\SI{1}{paper unit}, Dimensions: \si{\milli\meter}}
	\label{fig:app:autocad}
\end{figure}
